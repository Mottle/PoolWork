...
def train_model(model, classifier, train_loader, optimizer, criterion, device):
    ...
    for data in track(train_loader, description=f"Run train", disable=True):
        ...
        data = data.to(device)
        pooled, additional_loss = model(data.x, data.edge_index, data.batch)
...
def test_model(model, classifier, test_loader, criterion, device):
    model.eval()
    classifier.eval()
...
def run(config: BenchmarkConfig):
    ...
    for dataset in track(datasets(config.use_simple_datasets), description="All Datasets"):
        ...
        if config.use_simple_datasets:
            save_result([(f'{dataset}', all, last, last10, last50)], f'./over_smooth/result_simple/{config.model}.txt', dataset_end - dataset_start, config, id == 0)
        else:
            save_result([(f'{dataset}', all, last, last10, last50)], f'./over_smooth/result/{config.model}.txt', dataset_end - dataset_start, config, id == 0)
    all_end = get_time_sync()
