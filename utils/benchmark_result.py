...
class BenchmarkResult:
    ...
    def format(self, name: str = None, percent: bool = True, last = None) -> str:
        ...
        if percent == True:
            return f'{name}: {self.get_mean(last) * 100:.2f}% ± {self.get_std(last) * 100:.2f}% (best = {self.get_max(last) * 100:.2f}%)'
        else:
            return f'{name}: {self.get_mean(last):.4f} ± {self.get_std(last):.4f} (best = {self.get_max(last):.4f})'
